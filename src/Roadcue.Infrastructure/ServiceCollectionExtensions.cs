...
{
    public static IServiceCollection AddRoadcueDestinations(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKey)) services.AddSingleton<IDestinationGeocoder, StubDestinationGeocoder>();
        else services.AddHttpClient<IDestinationGeocoder, HereDestinationGeocoder>((sp, client) =>
        {
            client.BaseAddress = new Uri(configuration[$"{HereOptions.SectionName}:GeocodingBaseUrl"] ?? "https://geocode.search.hereapi.com");
            client.Timeout = TimeSpan.FromSeconds(10);
        });
        return services;
    }

    public static IServiceCollection AddRoadcueLocation(this IServiceCollection services, IConfiguration configuration)
    {
        services.AddSingleton(TimeProvider.System);
    ...
    public static IServiceCollection AddRoadcueTrips(this IServiceCollection services)
    {
        services.AddScoped<ITripService, TripService>();
        return services;
    }

    public static IServiceCollection AddRoadcueSpeech(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (string.IsNullOrWhiteSpace(key) || string.IsNullOrWhiteSpace(region)) services.AddSingleton<ISpeechSynthesizer, StubSpeechSynthesizer>();
        else
        {
            services.AddSingleton<IAzureSpeechClient, AzureSpeechClient>();
            services.AddSingleton<ISpeechSynthesizer>(sp => new AzureSpeechSynthesizer(sp.GetRequiredService<IAzureSpeechClient>(), sp.GetRequiredService<ILogger<AzureSpeechSynthesizer>>()));
        }
        return services;
    }
}
